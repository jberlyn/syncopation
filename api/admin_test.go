...
func TestAdminUI(t *testing.T) {
	...
	// 1. Zero users - access / should redirect to /setup
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	w := httptest.NewRecorder()
	mux.ServeHTTP(w, req)
	if w.Code != http.StatusFound || w.Header().Get("Location") != "/setup" {
		t.Fatalf("Expected redirect to /setup, got %d %s", w.Code, w.Header().Get("Location"))
	}
	...
	if w.Code != http.StatusFound || w.Header().Get("Location") != "/" {
		t.Fatalf("Expected redirect to / after setup, got %d %s", w.Code, w.Header().Get("Location"))
	}

	// 3. User now exists, /setup should redirect to /
	req = httptest.NewRequest(http.MethodGet, "/setup", nil)
	w = httptest.NewRecorder()
	mux.ServeHTTP(w, req)
	if w.Code != http.StatusFound || w.Header().Get("Location") != "/" {
		t.Fatalf("Expected redirect to / when setup already complete, got %d %s", w.Code, w.Header().Get("Location"))
	}
	...
	if w.Code != http.StatusFound || w.Header().Get("Location") != "/" {
		t.Fatalf("Expected redirect to / after successful login, got %d", w.Code)
	}
	...
	if w.Code != http.StatusOK {
		t.Fatalf("Expected 200 OK when accessing / with valid session, got %d", w.Code)
	}
}
